...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <html lang="en">
      <body className="min-h-screen bg-studio-950 font-sans text-[#eef3ef]">
        <main className="flex min-h-screen items-center justify-center px-6">
          <section className="max-w-xl rounded-lg border border-red-300/30 bg-studio-850 p-6">
            <div className="flex items-center gap-3 text-xs uppercase tracking-[0.18em] text-red-300">
              <AlertTriangle className="h-5 w-5" />
              Global Error
            </div>
            <h1 className="mt-4 text-2xl font-semibold">GreenLight could not finish loading.</h1>
